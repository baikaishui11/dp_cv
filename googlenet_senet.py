...
class SeModule(nn.Module):
    def __init__(self, in_channels, r=16):
        ...
        self.fc1 = nn.Conv2d(in_channels, in_channels // r, kernel_size=(1, 1), stride=1)
        self.fc2 = nn.Conv2d(in_channels // r, in_channels, kernel_size=(1, 1), stride=1)
...
class SeModuleV3(nn.Module):
    def __init__(self, in_channels, r=16):
        super(SeModuleV3, self).__init__()
        self.fc1 = nn.Conv2d(in_channels, in_channels // r, kernel_size=(1, 1), stride=1)
        self.fc2 = nn.Conv2d(in_channels // r, in_channels, kernel_size=(1, 1), stride=1)
        self.pool = nn.MaxPool2d((5, 5), 1, padding=2)
        self.bias = nn.Parameter(torch.zeros([1, in_channels, 1, 1]))

    def forward(self, x):
        alpha = F.relu(self.fc1(x))
        alpha = torch.sigmoid(self.fc2(alpha))
        alpha = self.pool(-1.0 * alpha) * -1.0
        alpha = F.relu(alpha + F.tanh(self.bias)) / 2.0
        x = x * alpha
        return x
...
class SeNetIInception(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(SeNetIInception, self).__init__()
        self.block = Inception(in_channels, out_channels)
        se_in_channels = int(sum([i[-1] for i in out_channels]))
        self.se_block = SeModule(se_in_channels)
...
class SeNetIInceptionV2(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(SeNetIInceptionV2, self).__init__()
        self.block = Inception(in_channels, out_channels)
        se_in_channels = int(sum([i[-1] for i in out_channels]))
        self.se_block = SeModuleV2(se_in_channels)
...
def t1():
    ...
    traced_script_module.save("./output/models/googlenet_inception.pt")
    torch.save(net, "./output/models/googlenet.pkl")
    torch.onnx.export(
        model=net.eval().cpu(),
        args=_x,
        f="./output/models/googlenet_inception.onnx",
        input_names=["images"],
        output_names=["scores"],
        opset_version=12,
        dynamic_axes={
